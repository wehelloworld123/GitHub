...
(function($) {
	$.extend({
		...
		operate:{
            ...
            look: function(id) {
                var url = $.table._option.lookUrl.replace("{id}", id);
                $.modal.open("查看" + $.table._option.modalName, url);
            },
            lose: function(id) {
                console.log(id);
                $.modal.confirm("确定打回该条" +$.table._option.modalName+ "信息吗？", function() {
                    var url = $.common.isEmpty(id) ? $.table._option.loseUrl : $.table._option.loseUrl.replace("{id}", id);
                    var data = {
                        "ids": id
                    };
            ...
            apply:function(id){
                $.modal.confirm("确定上线该部" +$.table._option.modalName+ "作品吗？", function() {
            ...
            uploadFile: function() {
                ...
                if (rows.length == 0||row.length>1) {
                    $.modal.alertWarning("只能选择一条记录");
                    return
                }
                $.modal.confirm("确认要下载选中的" + rows.length + "数据吗?", function() {
                    var url = $.table._option.fileUrl;
                    var data = {
                        "ids": rows.join()
                    };
                    $.operate.submit(url, "post", "json", data);
